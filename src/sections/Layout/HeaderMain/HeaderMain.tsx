...
const navigationItems = [
    { label: "Home", icon: HomeIcon, path: "/family" },
    { label: "Search", icon: SearchIcon, path: "/family/care-givers" },
    { label: "Messages", icon: MessageCircleIcon, path: "/family/messages" },
    { label: "Jobs", icon: BriefcaseIcon, path: "/family/jobs" },
    { label: "Favorites", icon: HeartIcon, path: "/family/wishlist" },
];

const profileDropdown = [
    { label: "Jobs", path: "/family/jobs" },
    { label: "Account & Settings", path: "/family/account-settings" },
    { label: "Help", path: "/family/help-center" },
...
export const HeaderMain = (): JSX.Element => {
   ...
   const handleDropDown = (label: string, path: string | null) => {
    ...
    return (
        <header className={`flex flex-col w-full ${location.pathname === "/family" ? "bg-[#fffff4]" :"bg-white"}`}>
                            ...
                            {navigationItems.map((item, i) => {
                                ...
                                return (
                                    <Link
                                        key={i}
                                        to={item.path}
                                        className={`flex flex-col items-center gap-1 text-sm [font-family:'Poppins',Helvetica] 
                            ...
                            <div className="relative" ref={profileRef}>
                                ...
                                >
                                    <Avatar className="w-[34px] h-[34px] bg-gray-100 p-2 rounded-full">
                                        <AvatarFallback className="[font-family:'Poppins',Helvetica] font-medium text-primary-11 text-xs">
                                            CA
                                        </AvatarFallback>
                                    </Avatar>
                                    <ChevronDownIcon
 ...
 to={item.path || "#"}
    onClick={(e) => {
        ...
        handleDropDown(item.label, item.path);
                                                        ...
                                                        className="[font-family:'Poppins',Helvetica] block w-full text-left text-[14px] text-black hover:bg-gray-100 p-2 rounded-md hover:text-primary-1 focus:outline-none"
                                                    >
                                                        {item.label}
                                                    </Link>
                                                </li>
                            ...
                            <Button onClick={() => navigate("/family/subscription")} className="h-12 px-8 py-2 bg-primary-11 rounded-xl [font-family:'Geist',Helvetica] font-bold text-white text-lg hover:bg-primary-11/90">
                                Upgrade
                            </Button>
                        </div>
                    </div>
    ...
    onClick={(e) => {
        ...
        handleDropDown(item.label, item.path);
                                                    ...
                                                    className="[font-family:'Poppins',Helvetica] block w-full text-left text-[13px] text-black hover:bg-gray-100 p-1 rounded-md focus:outline-none"
                                                >
                                                    {item.label}
                                                </Link>
                                            </li>
                        ...
                        <div className="fixed inset-0 z-[9999] lg:hidden" role="dialog">
                            ...
                            <div className="fixed left-0 top-0 h-full w-[80%] max-w-[320px] bg-white shadow-xl">
                                ...
                                <ul className="flex flex-col p-6 space-y-3 mt-12">
                                    {navigationItems.map((item, i) => (
                                        <li key={i}>
                                            <Link
                                                to={item.path}
                                                onClick={() => setIsMenuOpen(false)}
                                                className="flex items-center gap-3 text-base py-2 px-2 [font-family:'Poppins',Helvetica] text-heading hover:text-primary-11"
                                            >
                                                <item.icon className="w-6 h-6" />
                                                {item.label}
                                            </Link>
                                    ...
                                    <li>
                                        <Button onClick={() => navigate("/family/subscription")} className="h-12 w-full mt-4 bg-primary-11 text-white text-base rounded-xl hover:bg-primary-11/90">
                                            Upgrade
                                        </Button>
                                    </li>
                                </ul>
